...
const COGNITO = {
  userPoolId: 'us-east-1_W2PqBh0S6',
  clientId: '4u30fvs0fjtet6l7fiduhm7964',
  clientSecret: null,
  domain: 'https://us-east-1w2pqbh0s6.auth.us-east-1.amazoncognito.com',
  redirectUri: 'http://pethq-alb-557436987.us-east-1.elb.amazonaws.com/callback',
  logoutUri: 'http://pethq-alb-557436987.us-east-1.elb.amazonaws.com',
  region: 'us-east-1'
};
...
// POST /api/chat — MCP Orchestration (Direct Ollama — replaces Dify)
if (path==='/api/chat' && req.method==='POST') {
  try {
    ...
    // ── Direct Ollama call (replaces Dify in portfolio version) ──
    const ollamaBody = JSON.stringify({
      model: 'tinyllama',
      messages: [
        { role: 'system', content: 'You are a helpful pet care assistant. Answer in 1-3 sentences. Be direct and use the provided context.' },
        { role: 'user', content: contextSummary ? `[Context: ${contextSummary}] ${message}` : message }
      ],
      stream: false
    });
    ...
    const ollamaRes = await new Promise((resolve, reject) => {
      ...
      ollamaReq.setTimeout(180000, () => { ollamaReq.destroy(); reject(new Error('timeout')); });
      ollamaReq.write(ollamaBody);
      ollamaReq.end();
    });

    const aiResponse = ollamaRes.message?.content || 'No response from AI.';
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ response: aiResponse, tools_invoked: toolsInvoked }));
  } catch(err) {
    res.writeHead(500, { 'Content-Type': 'application/json' });
